...
This file provides guidance to AI coding agents (Claude Code, Codex 等) when working with code in this repository.
...
- `[[claude_marketplaces]]` / `[[claude_plugins]]`: `chezmoi apply` 時に `claude plugin install` で自動インストール（`dot_claude/run_onchange_install-plugins.sh.tmpl`）
  - marketplace: `name`（登録名 = `.claude-plugin/marketplace.json` の `name`）, `source`（`owner/repo`）
  - plugin: `marketplace` に登録名を参照（例: `openai/codex-plugin-cc` の登録名は `openai-codex`）
- `[[codex_marketplaces]]` / `[[codex_plugins]]`: `chezmoi apply` 時に `codex plugin add` で自動インストール（`dot_codex/run_onchange_install-plugins.sh.tmpl`）
  - **Claude pluginとは別機構**（`codex plugin` CLI、設定先 `~/.codex`）。混在させない
  - marketplace名はソースの `.agents/plugins/marketplace.json` の `name` と一致させる
  - `reserved = true` は Codex 同梱の予約marketplace（例: `openai-curated`）。`codex login` 後に利用可能になり、`marketplace add` はスキップされる
...
- **Codex agents**: `~/.codex/agents/{name}.md`
