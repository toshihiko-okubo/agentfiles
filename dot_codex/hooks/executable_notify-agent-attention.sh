...
case "$(uname -s 2>/dev/null || printf unknown)" in
  Darwin)
    run_if_available afplay /System/Library/Sounds/Glass.aiff && exit 0
  ...
  Linux)
    ...
    run_if_available play -q -n synth 0.18 sine 880 vol 0.20 && exit 0
    run_if_available speaker-test -t sine -f 880 -l 1 && exit 0
    ;;
esac
